...
2. **Launch the program in a detached tmux pane** at the chosen size. tmux gives a stable, re-attachable, correctly-sized pane; naming it lets you kill it cleanly later:
   ...
   ```
   Use a collision-free `<name>` (e.g. your Orbh session's short id). Set the size on the tmux session, not on ttyd — the pane geometry is what the program reads.

3. **Serve the pane over HTTP with ttyd**, bound to localhost and **writable** (ttyd is read-only by default — without `-W` your keystrokes are silently dropped). Run it in the background:
   ```bash
   ttyd -p <port> -i 127.0.0.1 -W tmux attach -t <name>
   ```
   `-W`/`--writable` enables client input; `-i 127.0.0.1` keeps it off the network. Note the URL: `http://127.0.0.1:<port>`.
...
4. **Open and drive it through the browser** via [[dev-sk-ab-use_browser]] — claim a browser, `open http://127.0.0.1:<port>`, and drive the `xterm.js`:
   - **Read the screen with screenshots, not snapshots.** The terminal is a `<canvas>`, so `snapshot -i` won't return cell text — use `agent-browser --session <b> screenshot <path>` and read the grid visually. Re-screenshot after every action.
   - **Focus the terminal before typing** (click the terminal canvas once), then send keys: `agent-browser --session <b> type "..."`, `press Enter|Escape|Tab|ctrl+c`, or `keyboard` for chords/arrows.
   - **Mouse works for real:** clicks and wheel go through `xterm.js` and are reported to the program only if it enabled mouse tracking — which is exactly the behavior you want to verify. A click at a pane cell lands where the program thinks it does.
...
7. **Clean up — kill the harness, close and release the browser.** Leaving ttyd/tmux/Chrome running clogs the machine and ports:
   ```bash
   # stop ttyd (the backgrounded process from step 3), then:
